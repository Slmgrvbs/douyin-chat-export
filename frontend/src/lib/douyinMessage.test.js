...
import {
  ...
  isLooseEmoji, isLooseShare, isLooseImage, isShareCard, isPosterShare,
...
describe('leftover media payload display', () => {
  ...
  it('renders poi / awe 805 / 2104 as share cards', () => {
    ...
    expect(isPosterShare(work)).toBe(true)
    expect(isPosterShare(live)).toBe(true)
    expect(isPosterShare(poi)).toBe(true)
    expect(isPosterShare(withCj({ im_dynamic_patch: { raw_data: '{}' } }, { msg_type: 4 }))).toBe(false)
  })
